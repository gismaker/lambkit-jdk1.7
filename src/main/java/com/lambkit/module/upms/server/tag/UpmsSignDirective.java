 ...
 * upms_sign标签<br>
 * 参数：{id:主键}
 * 返回值：{entity:upms_sign信息}
 * @author lambkit
 */
public class UpmsSignDirective extends BaseDirectiveModel {
	...
	public void executeMe(Environment env, Map params, TemplateModel[] loopVars,
		...
		String stime = get(params, "stime");
		String lng = get(params, "lng");
		String lat = get(params, "lat");
		String sway = get(params, "sway");
		int pagenum = getInt(params, "pagenum", 0);
		int pagesize = getInt(params, "pagesize", 0);
		String wheresql = get(params, "sql", null);
		String sql = " from upms_sign where "; 
		if(wheresql == null) {
			sql += " 1=1 ";
			if(StringUtils.hasText(id)) sql += " and id=" + id;//int unsigned
			if(StringUtils.hasText(userId)) sql += " and user_id=" + userId;//int unsigned
			if(StringUtils.hasText(stime)) sql += " and stime like '%" + stime + "%'";//datetime
			if(StringUtils.hasText(lng)) sql += " and lng=" + lng;//float
			if(StringUtils.hasText(lat)) sql += " and lat=" + lat;//float
			if(StringUtils.hasText(sway)) sql += " and sway like '%" + sway + "%'";//varchar
		} else {
			sql += wheresql;
		}
		...
		UpmsSignService service = UpmsSign.service();
