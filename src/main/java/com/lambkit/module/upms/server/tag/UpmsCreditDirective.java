 ...
 * upms_credit标签<br>
 * 参数：{id:主键}
 * 返回值：{entity:upms_credit信息}
 * @author lambkit
 */
public class UpmsCreditDirective extends BaseDirectiveModel {
	...
	public void executeMe(Environment env, Map params, TemplateModel[] loopVars,
		...
		String credit = get(params, "credit");
		String ctype = get(params, "ctype");
		...
		String sql = " from upms_credit where "; 
		if(wheresql == null) {
			sql += " 1=1 ";
			if(StringUtils.hasText(id)) sql += " and id=" + id;//int unsigned
			if(StringUtils.hasText(userId)) sql += " and user_id=" + userId;//int unsigned
			if(StringUtils.hasText(credit)) sql += " and credit=" + credit;//int
			if(StringUtils.hasText(ctype)) sql += " and ctype like '%" + ctype + "%'";//varchar
			if(StringUtils.hasText(ctime)) sql += " and ctime like '%" + ctime + "%'";//datetime
		...
		UpmsCreditService service = UpmsCredit.service();
